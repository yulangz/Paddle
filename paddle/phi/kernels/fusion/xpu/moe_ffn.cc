...
void MoeFFNKernel(const Context& ctx,
  ...
  auto ffn1_input = baidu::xpu::xft::xftTensor<XPUType, 2>(
      reinterpret_cast<XPUType*>(
          const_cast<T*>(permute_input.data<T>())),
      std::array<int64_t, 2>{permute_input.dims()[0],
                             permute_input.dims()[1]});
  ...
  XPUType* ffn1_out_paddle = RAII_GUARD.alloc_l3_or_gm<XPUType>(expanded_active_expert_rows * inter_size);
  auto ffn1_out_tensor = baidu::xpu::xft::xftTensor<XPUType, 2>(
      reinterpret_cast<XPUType*>(ffn1_out_paddle),
      std::array<int64_t, 2>{expanded_active_expert_rows,
                             inter_size});
  ...
  int32_t* sorted_tokens_idx_paddle = RAII_GUARD.alloc_l3_or_gm<int32_t>(expanded_active_expert_rows);
  ...
  if (quant_method == "weight_only_int4") {
    auto ffn1_w_tensor = baidu::xpu::xft::xftMat<int4_t>(
        reinterpret_cast<int4_t*>(const_cast<int8_t*>(ffn1_weight.data<int8_t>())),
        nullptr,
  ...
  XPUType* ffn2_input_paddle = RAII_GUARD.alloc_l3_or_gm<XPUType>(expanded_active_expert_rows * inter_size / 2);
  ...
  auto ffn2_out_tensor = baidu::xpu::xft::xftTensor<XPUType, 2>(
      ...
      std::array<int64_t, 2>{ffn_out->dims()[0],
                             ffn_out->dims()[1]});
  if (quant_method == "weight_only_int4") {
    auto ffn2_w_tensor = baidu::xpu::xft::xftMat<int4_t>(
        reinterpret_cast<int4_t*>(const_cast<int8_t*>(ffn2_weight.data<int8_t>())),
        nullptr,
...
PD_REGISTER_KERNEL(
    moe_ffn, XPU, ALL_LAYOUT, phi::fusion::MoeFFNKernel, phi::dtype::float16, phi::dtype::bfloat16) {}
