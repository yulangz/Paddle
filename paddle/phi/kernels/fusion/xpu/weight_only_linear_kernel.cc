  ...
  if (dev_ctx.x_context()->dev().type() == baidu::xpu::api::kXPU3) { 
    ...
    if(weight_dtype == "int4") {
      ...
      baidu::xpu::xblas::FcFusionTensor<const XPUType> tensor_w{
          ...
          reinterpret_cast<const XPUType*>(weight.data<T>()), nullptr, n, k, k, true};
      int r1 =  baidu::xpu::xblas::fc_fusion<XPUType,
                                          XPUType,
                                          XPUType,
                                          XPUType,
    ...
    if (!weight_scale.is_initialized()) {
      PADDLE_THROW(common::errors::Fatal(
          "WeightOnlyLinearKernel xpu need weight_scale as input for xpu2."));
    }
    auto weight_scale_tensor = weight_scale.get();
    ...
    switch (x.dtype()) {
      case phi::DataType::FLOAT16: {
        ...
        max_value.Resize(weight_scale_tensor.dims());
        dev_ctx.template Alloc<float>(&max_value);
        if (weight_scale_tensor.dtype() == phi::DataType::FLOAT16) {
          DenseTensor max_value_fp16;
          max_value_fp16.Resize(weight_scale_tensor.dims());
          ...
          r = baidu::xpu::api::cast<XPUType, float>(
              ...
              max_value.numel());
          PADDLE_ENFORCE_XDNN_SUCCESS(r, "cast");
        } else if (weight_scale_tensor.dtype() == phi::DataType::FLOAT32) {
